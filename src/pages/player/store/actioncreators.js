/*
 * @Author: 牛皓
 * @Date: 2020-11-10 10:28:42
 * @LastEditors: 牛皓
 * @LastEditTime: 2020-11-12 15:22:14
 * @FilePath: \wx-music\src\pages\player\store\actioncreators.js
 */
...
export const changePlaySongAction = (tag) =>{//更改歌曲
    return (dispatch,getState) => {
        ...
        switch(playSequence) {
            case 1:
                currentSongIndex = Math.floor(Math.random() * playList.length);//随机播放一首
                break;
            default:
                currentSongIndex += tag;
                if(currentSongIndex === playList.length) currentSongIndex = 0;
                if(currentSongIndex === -1) currentSongIndex = playList.length - 1;//如果传入的tag是-1，则在playList里面找到最后一位
        }
